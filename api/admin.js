...
function updateList(html, cases) {
  html = pageSeo.withMetadata(html || "", pageSeo.LIST_METADATA);
  html = html.replace(
    /<!-- ADMIN-CASE:[\s\S]*?<!-- \/ADMIN-CASE:[^>]*-->\s*/g,
    "",
  );
  const cards = [...cases]
    ...
    .map(card)
    .join("\n");
  html = html.replace(/(<div class="wrap board-list">)\s*/, `$1\n${cards}\n`);
...
function serviceHub(cases, district, serviceKey) {
  ...
  const seoTitle = `${service} ${district} ${seo?.intent || "실제 작업"}`;
...
function updateRss(cases) {
  const items = [...cases]
    ...
    .map((c) => {
      ...
      return `<item><title>${xml(pageSeo.caseMetadata(c).title)}</title><link>${url}</link><guid isPermaLink="true">${url}</guid><pubDate>${new Date(`${c.date}T00:00:00+09:00`).toUTCString()}</pubDate><description>${xml(pageSeo.caseMetadata(c).description)}</description><content:encoded><![CDATA[${body.replaceAll("]]>", "]]]]><![CDATA[>")}]]></content:encoded></item>`;
    })
    .join("\n");
