...
const BRAND = "신라건축설비";
const MAIN_TITLE = "신라건축설비 | 서울·경기·인천 하수구업체 24시";
const MAIN_DESCRIPTION = "서울·경기·인천 하수구업체 신라건축설비. 변기막힘·싱크대막힘·하수구역류부터 누수탐지·고압세척·배관공사까지, 현장 진단 후 작업 범위와 비용을 안내합니다. 24시간 상담.";
const SERVICES = {
...
function caseMetadata(c) {
  ...
  // Case titles should read naturally to customers while keeping the exact local service keyword once.
  // Example: "서초구 서초동 변기막힘 | 변기를 뜯어보니 마라탕 재료가 걸려 있었습니다"
  const prefix = clean(`${region} ${service}`);
  const subject = stripContext(c.title, c) || stripContext(c.result, c);
  const room = Math.max(1, 54 - [...prefix].length - [...BRAND].length - 5);
...
function serviceMetadata(key, district = "", count) {
  if (key === "sink-clog" && district === "강남구") return {
    title: "강남 싱크대막힘 원인 진단과 해결사례 | 신라건축설비",
    description: "강남구 싱크대막힘과 주방 배수구 역류, 반복되는 기름때 막힘을 진단합니다. 실제 현장사례로 석션·샤프트·고압세척의 적용과 비용 결정 기준을 확인하세요.",
  };
